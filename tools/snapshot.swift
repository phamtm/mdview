...
guard args.count > 3 else { fatalError("usage: snapshot <resources> <md> <out-prefix> [light|dark]") }
...
final class Runner: NSObject, WKNavigationDelegate {
    ...
    override init() {
        ...
        webView = WKWebView(frame: NSRect(x: 0, y: 0, width: 900, height: 1200), configuration: config)
        window = NSWindow(contentRect: NSRect(x: -20000, y: -20000, width: 900, height: 1200),
    ...
    func runFrontmatterTests() {
        let script = (try? String(contentsOf: URL(fileURLWithPath: "tools/frontmatter-tests.js"),
                                  encoding: .utf8)) ?? ""
        guard !script.isEmpty else { print("FRONTMATTER {\"failures\":[\"test file missing\"]}"); shoot(); return }
        webView.evaluateJavaScript(script) { value, error in
            if let error { print("FRONTMATTER {\"failures\":[\"\(error)\"]}") }
            else if let s = value as? String { print("FRONTMATTER \(s)") }
            self.shoot()
        }
    }
                ...
                self.webView.takeSnapshot(with: config) { image, error in
                    ...
                    if let image, let tiff = image.tiffRepresentation,
                       ...
                       let png = rep.representation(using: .png, properties: [:]) {
                        ...
                        print("wrote \(out.path) \(Int(image.size.width))x\(Int(image.size.height))")
                    }
                    app.terminate(nil)
                }
            }
